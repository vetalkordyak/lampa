(function() {
    ...
    const menuItems = [
        { action: "hd", text: "В качестве", url: "?cat=&sort=now&uhd=true&language=ua", title: "В качестве", component: "category_full", source: "cub", sort: "now", card_type: "true", page: 1 },
        { action: "netflix", text: "Netflix", url: "discover/tv?language=ua&with_networks=213", title: "Netflix", component: "category_full", source: "tmdb", sort: "now", card_type: "true", page: 1 },
    ];

    function createMenuItem(item) {
        const menuItem = $(`<li class="menu__item selector" data-action="pact_${item.action}"><div class="menu__ico">${svgIcon}</div><div class="menu__text">${item.text}</div></li>`);
